<template>
  <div  class="mt-6">
    <div v-for="(q,index) in allQuestions" :key="index">
      <div class="w-2/5 p-6 bg-gray-600 rounded-lg">
        <h1>{{q.question}}</h1>
        <div class="mt-2 grid grid-cols-2 gap-4 justify-items-center border border-white border-1">
          <div v-for="(an,i) in q.answer" :key="i" class="w-40 p-4 bg-green-600 rounded-lg cursor-pointer text-center">
            <h2>{{an}}</h2>
          </div>
        </div>
      </div>
