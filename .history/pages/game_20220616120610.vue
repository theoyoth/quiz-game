      ...
      <div v-for="(q,i) in questionNow" :key="i" class="mb-4 xl:w-1/2 lg:w-3/4 sm:w-11/12 md:w-2/3">
        <div class="px-6 py-8 bg-gray-600 rounded-lg text-center">
          ...
          <div class="mt-4 grid grid-cols-2 gap-4 justify-items-center">
            <div v-for="(an,i) in q.answer" :key="i" class="w-40 py-2 bg-green-600 rounded-lg cursor-pointer text-center hover:bg-green-500" @click="sendAnswer" ref="val">
              {{an}}
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    sendAnswer(e){
      ...
      // this.$ref.val.classList.add(classApply)
      target.classList.add(classApply)

      setTimeout(() => {
        target.classList.remove(classApply)
