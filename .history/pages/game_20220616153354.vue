<template>
  <div  class="mt-6">
    ...
    <div v-else class="w-full grid grid-cols-1 place-items-center border border-white border-1">
      <div class="xl:w-1/2 lg:w-3/4 sm:w-11/12 md:w-2/3 border border-white border-1">
        <div class="w-full flex justify-between">
          <div class="bg-gray-600">score</div>
          <div class="bg-gray-600">score</div>
        </div>
        <div v-for="(q,i) in questionNow" :key="i" class="mb-4 border border-white border-1">
          <div class="px-6 py-8 bg-gray-600 rounded-lg text-center">
            ...
            <div class="mt-4 grid grid-cols-2 gap-4 justify-items-center">
              <div v-for="(an,i) in q.answer" :key="i" class="w-40 py-2 bg-green-600 rounded-lg cursor-pointer text-center hover:bg-green-500" @click="sendAnswer">
                {{an}}
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    ...
    sendAnswer(e){
      ...
      target.parentElement.classList.add(classApply)

      setTimeout(() => {
        target.parentElement.classList.remove(classApply)
