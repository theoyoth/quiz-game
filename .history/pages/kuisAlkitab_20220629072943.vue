        ...
        <div class="mb-4" v-for="(q,i) in questionNow" :key="i">
          <div class="px-6 py-8 bg-gray-600 rounded-lg text-center">
            ...
            <div class="mt-4 grid grid-cols-2 gap-4 justify-items-center">
              <div v-for="(an,i) in q.choices" :key="i" class="sm:w-36 w-44 px-2 py-2 bg-green-600 rounded-lg cursor-pointer text-center hover:bg-green-500" @click="sendAnswer">
              {{an}}
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    async getKuisAlkitab(){
      ...
      // const data = await require('../kuis.json');
      // const dataFetch = await fetch('./kuis.json');
      // console.log(dataFetch)
      const data = await this.$axios.get('./kuis.json')
      this.allQuiz = data.data
